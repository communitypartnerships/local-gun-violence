...
**[One-Page Summary](/pdfs/gv-onepager-2025.pdf){.external target="_blank"}**
...
```{r collective_toll, fig.cap="Incidents of Gun Violence in Charlottesville & Albemarle and Number of Victims by Month"}
gva <- read_csv("data/gva_incidents_24.csv") %>%
  clean_names() %>%
...
```{r}
vdh_intent <- read_csv("cville-alb-gun-trauma/data/vdh_deaths_intent.csv") %>%
...
labs(title= "Firearm Deaths in the Blue Ridge Health District (2018-2022)",
...
```{r vdh_injuries_plt}
vdh_injuries <- read_csv("cville-alb-gun-trauma/data/vdh_injury_county.csv") %>%
  clean_names() %>%
...
```{r vdh_age_plt}
vdh_deaths_age <- read_csv("cville-alb-gun-trauma/data/vdh_deaths_age.csv") %>%
...
gva_incidents <- read_csv("cville-alb-gun-trauma/data/gva_incidents.csv") %>%
  mutate(incident_date = mdy(incident_date)) %>%
  select(-operations) %>%
  filter(incident_date <= "2024-12-31")

#Participants in Cville/Albemarle
gva_participants <- read_csv("cville-alb-gun-trauma/data/gva_participants.csv") %>%
  mutate(incident_date = ymd(incident_date)) %>%
  filter(incident_date <= "2024-12-31")

#Incidents in Virginia
gva_incidents_va <- read_csv("cville-alb-gun-trauma/data/gva_incidents_virginia.csv") %>%
  select(-incident_id, -incident_date) %>%
  filter(date <= "2024-12-31")

#Participants in Virginia
gva_participants_va <- read_csv("cville-alb-gun-trauma/data/gva_participants_virginia.csv") %>%
  filter(date <= "2024-12-31")
...
gva_incidents %>%
  ...
  labs(x = "Year",
       y = "Number of Victims",
       title = "Victims of Gun Violence in the Charlottesville/Albemarle Region",
...
gva_incidents_va %>%
  ...
  labs(x = "Year",
       y = "Number of Victims",
       title = "Victims of Gun Violence in Virginia",
       caption = "Data Source: Gun Violence Archive") +
  scale_fill_manual(labels = c("Injured", "Killed"),
                    values = c("#F8BE3D", "#007BAB"),
                    guide = guide_legend(title = "Victim Status")) +
...
```{r gva_participants_cville, fig.cap="Ages of Individuals Involved in Gun Violence in the Charlottesville/Albemarle Region"}
gva_participants <- read_csv("cville-alb-gun-trauma/data/gva_participants.csv") %>%
  drop_na(age) %>%
...
ggplot(age_dat, aes(n_tot, age_grp, fill = role)) +
  ...
  labs(y = "Age Range",
       ...
       title = "Ages of Individuals Involved in Gun Violence in the Charlottesville/Albemarle Region",
       caption = "Data Source: Gun Violence Archive") +
  theme_minimal() +
...
```{r suicide_region}
cdc_deaths_intent <- read_csv("cville-alb-gun-trauma/data/cdc_deaths_intent.csv")
...
suicide_age_pcts <- read_csv("cville-alb-gun-trauma/data/cdc_suicide_age.csv") %>%
  select(-...1) %>%
  filter(locality == "Blue Ridge Health District") %>%
  mutate(
    deaths = case_when(
...
race_brhd <- read_csv("cville-alb-gun-trauma/data/acs_race_brhd.csv")

cdc_suicide_race <- read_csv("cville-alb-gun-trauma/data/cdc_suicide_race.csv")
...
race_va <- read_csv("cville-alb-gun-trauma/data/acs_race_va.csv")
...
```{r domestic_local}
domestic_local <- read_csv("cville-alb-gun-trauma/data/nibrs_domestic_firearm.csv") %>%
...
```{r domestic_va}
domestic_va <- read_csv("cville-alb-gun-trauma/data/nibrs_domestic_firearm.csv") %>%
  select(-acquaintance) %>%
  filter(locality == "Virginia")
...
```{r ipv}
ipv <- read_csv("cville-alb-gun-trauma/data/nibrs_ipv.csv")
...
firearm_age <- read_csv("cville-alb-gun-trauma/data/nibrs_firearm_age.csv")
...
```{r police_incidents, fig.cap="Incidents of Gun Violence in Charlottesville & Albemarle"}
...
ggplot(years, aes(yr, count, fill = type)) +
  ...
  labs(x = "Year",
       ...
       title = "Incidents of Gun Violence in Charlottesville & Albemarle",
...
```{r victim_demos}
race_county <- read_csv("cville-alb-gun-trauma/data/acs_race_county.csv")

demos <- read_excel("data/Regional GV Data - 2019-2024 YTD.xlsx", sheet = "Demographic Data for Cases") %>%
  clean_names() %>%
  distinct() %>%
...
odp_crimes <- read_csv("cville-alb-gun-trauma/data/odp_crimes.csv")
odp_arrests <- read_csv("cville-alb-gun-trauma/data/odp_arrests.csv")
load(file = "cville-alb-gun-trauma/data/cville_map.RData")
...
```{r odp_arrests, fig.cap="Number of Firearm-Related Arrests per Person"}
...
odp_arrests %>%
  ...
  labs(x = "Number of People",
       ...
       title = "Number of Firearm-Related Arrests per Person",
...
```{r force}
force <- read_csv("cville-alb-gun-trauma/data/nibrs_force.csv")
```
...
```{r ois}
police_shootings <- read_csv("cville-alb-gun-trauma/data/gva_officer.csv") %>%
...
dat <- readRDS("cville-alb-gun-trauma/data/acs_tract_info.RDS")
...
```{r age_scatter, fig.cap="Under 18 Population Percentages Compared to Incidents of Gun Violence"}
...
```{r pov_scatter, fig.cap="Childhood Poverty Rates Compared to Incidents of Gun Violence"}
...
#### Are incidents of gun violence related to low personal earnings?
...
```{r earnings_scatter, fig.cap="Median Annual Earnings Compared to Incidents of Gun Violence"}
...
#### Are incidents of gun violence related to unemployment?
...
```{r employment_scatter, fig.cap="Unemployment Rates Compared to Incidents of Gun Violence"}
...
```{r atf_setup}
# # Local dealers 
local_ffl <- read_csv("cville-alb-gun-trauma/data/atf_local_licenses.csv")

# Albemarle map
load(file = "cville-alb-gun-trauma/data/alb_map.Rdata")
...
```{r dealer_map, out.width="90%", fig.cap="Locations of Firearms Dealers in Charlottesville & Albemarle"}
ggmap(alb_map) +
  ...
  labs(title = "Locations of Firearms Dealers in Charlottesville & Albemarle",
       caption = "Total federally licensed dealers: 19")
...
```{r nibrs_theft, fig.cap="Theft of Firearms from Vehicles"}
nibrs_theft <- read_csv("cville-alb-gun-trauma/data/nibrs_theft.csv") %>%
...
Samantha Toet, Michele Claibourn. “The State of Gun Violence in Charlottesville and Albemarle.” The UVA Center for Community Partnerships.
